...
typedef RT_MODEL_q_qube2_DOB_3x3_2020_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * q_qube2_DOB_3x3_2020b_new(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(q_qube2_DOB_3x3_2020b_new_M, GBLbuf.message);
}
  ...
  control_c_handler(int signal_number)
{
  ...
  GBLbuf.stopExecutionFlag = 1;
  rtExtModeStart();
  hil_task_stop(q_qube2_DOB_3x3_2020b_new_DW.HILReadEncoderTimebase_Task)
    ;
}
  ...
  main(int argc, char * argv[])
{
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv,
    "shmem://q_qube2_DOB_3x3_2020b_new:1");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: q_qube2_DOB_3x3_2020b_new -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
  ...
  if (rtExtModeQuarcStartup(rtmGetRTWExtModeInfo(S),
    ...
    if (rtmGetErrorStatus(S) == NULL) {
      ...
      result = hil_task_start
        (q_qube2_DOB_3x3_2020b_new_DW.HILReadEncoderTimebase_Task, (t_clock)
         q_qube2_DOB_3x3_2020b_new_P.HILReadEncoderTimebase_Clock, 500.0, -1)
        ;
      if (result == 0) {
